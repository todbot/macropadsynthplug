...
void setOscs() {  
  bool noteMode = false;
  for(int i=0; i<NUM_OSCS; i++) {
    float note = root_note + 24*(osc_vals[i]/99.0); 
    Q15n16 r = Q7n8_to_Q15n16(rand(200)); // random for oscillator "drift". 
    if( noteMode ) {      // need to fix this logic
      portamentos[i].start( (uint8_t)(note) );
    }
    else {
      portamentos[i].start( float_to_Q16n16(note) + r );  // + r
      // portamentos[i].start( float_to_Q16n16( note) + dnote*10 );  // + r
    }
  }  
}
