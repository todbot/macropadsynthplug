...
int8_t volume = 8; // volume is 4-bit: 0-15
...
int encoder_pos = volume; // our encoder position state
...
//    digitalWrite( led_pins[i], keypressed); // light up the key LEDs for yucks
  ...
  if (encoder_pos != newPos) {
    ...
    volume = (volume + (newPos-encoder_pos)) % 15;  // super hack
    encoder_pos = newPos;
  }
...
AudioOutput_t updateAudio() {
  ...
  asig += aOsc.next();
  asig *= volume;
  return MonoOutput::fromNBit(12, asig); // 8 sig, 4 volume
}
